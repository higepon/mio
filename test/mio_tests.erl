...
stats() ->
    {ok, Conn} = memcached:connect(?MEMCACHED_HOST, ?MEMCACHED_PORT),
    ok = memcached:set(Conn, "1001", "Hello"),
    {ok, [{"uptime", UptimeStr}, {"total_items", "1"}, {"cmd_get", "0"}, {"bytes", _Bytes}, {"cmd_get_multi_avg_time", _N}, {"cmd_get_multi_worst_time", _}, {"cmd_get_avg_time", _}, {"cmd_get_worst_time", _}, {"sweeped_items", _}]} = memcached:stats(Conn),
...
sweeper() ->
    ...
    ok = memcached:set(Conn, "mio:sweep", ""),
    ok = memcached:disconnect(Conn),
    timer:sleep(100),
    ...
    ?assertMatch({ok, [{"uptime", _},
                       ...
                       {"sweeped_items", "1"}]}, memcached:stats(Conn2)),
