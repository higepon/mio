...
link_on_level0(From, State, Self, Introducer) ->
    ...
    if
        ...
        IsSameKey ->
            ...
            io:format("Locked MyKey=~p ~p 1 ~n", [MyKey, [Neighbor]]),
            ...
            if IsDeleted ->
                    ...
                    unlock([Neighbor]),
                    io:format("UnLocked MyKey=~p ~p 1 ~n", [MyKey, [Neighbor]]),
                    link_on_level0(From, State, Self, Introducer);
               true ->
                    %% overwrite the value
...
link_on_level_ge1(Self, Level, MaxLevel) ->
    ...
    case node_on_level(MyLeft, LowerLevel) of
        ...
        %%  <Level>    : [D:m] <-> [F:m]
        [] ->
            link_on_level_ge1_no_left(Self, Level, MaxLevel, MyRight, MyKey, MyMV);
        %%     <Level - 1>: [A:m] <-> [B:n] <-> [NodeToInsert:m] <-> [C:n] <-> [D:m] <-> [E:n] <-> [F:m]
        %%     <Level>    : [A:m] <-> [D:m] <-> [F:m]
        LeftNodeOnLower ->
            ...
            case Buddy of
                [] ->
                    case node_on_level(MyRight, LowerLevel) of
                        ...
                        RightNodeOnLower2 ->
                            link_on_level_ge1_right(Self, Level, MaxLevel, MyKey, MyMV, RightNodeOnLower2)
                    end;
                %% <Level - 1>: [A:m] <-> [B:n] <-> [NodeToInsert:m] <-> [C:n] <-> [D:m] <-> [E:n] <-> [F:m]
                _ ->
...
link_on_level_ge1_buddy(Self, Level, MaxLevel, MyKey, Buddy, BuddyKey, BuddyRight, BuddyRightKey) ->
    ...
    io:format("Locked MyKey=~p ~p 7 ~n", [MyKey, [Self, Buddy, BuddyRight]]),
...
link_on_level_ge1_no_left(Self, Level, MaxLevel, MyRight, MyKey, MyMV) ->
    ...
    case Buddy of
        ...
        _ ->
            ...
            io:format("Locked MyKey=~p ~p 5 ~n", [MyKey, [Self, Buddy]]),
